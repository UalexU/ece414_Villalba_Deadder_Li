...
#include <hardware_gpio.h>       // Include for GPIO control functions from the Pico SDK
...
void led_out_write(uint8_t a) {
    ...
    while (true) {
        ...
        if (dir_left) {
            ...
            // We're shifting right (dir_left is false)
            // If the rightmost LED (outval = 0x1 or 00000001 in binary) is on, reverse direction
            if (outval == 0x1) {
                dir_left = true;         // Change direction to shift left
